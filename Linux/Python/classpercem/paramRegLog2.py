...
from libPoseDB import MongoConnT2A
import cv2
import numpy as np
...
def geraLinhaEspelho(ds):
    ...
    dangh = 2*abs(ds["angH"])
    respgl.append(dangh)
    respgl.append(abs(ds["angH"]))
    respgl.append(abs(ds["angH"]))
    respgl.append(abs(ds["angV"]))
    respgl.append(abs(ds["angV"]))
    dangv = 0.0
    respgl.append(dangv)
    return respgl
...
def geraLinhaDif(ds,ds2):
    ...
    respN.append(dangh)
    respN.append(abs(ds["angH"]))
    respN.append(abs(ds2["angH"]))
    respN.append(abs(ds["angV"]))
    respN.append(abs(ds2["angV"]))
    dangv = abs(ds["angV"]-ds2["angV"])
    respN.append(dangv)
    danghF = abs(ds["angH"]+ds2["angH"])
    respglF1.append(danghF)
    respglF1.append(abs(ds["angH"]))
    respglF1.append(abs(ds2["angH"]))
    respglF1.append(abs(ds["angV"]))
    respglF1.append(abs(ds2["angV"]))
    respglF1.append(dangv)
    respglF2.append(danghF)
    respglF2.append(abs(ds["angH"]))
    respglF2.append(abs(ds2["angH"]))
    respglF2.append(abs(ds["angV"]))
    respglF2.append(abs(ds2["angV"]))
...
if __name__=="__main__":
    ...
    for index, (name, logreg) in enumerate(classifiers.items()):
        ...
        mdb.salvaLogit(logreg, nomearq=name)
