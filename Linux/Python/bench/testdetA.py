        ...
        if ret:
            ...
            for x0,y0,x1,y1 in rectpts:
                ...
                if (x1-x0)/fatorg > 100 or (y1 - y0 )/fatorg > 100 :
                    grava = True
                cv2.rectangle(reduzido,(x0,y0),(x1,y1),(0,255,0),3)
            if grava:
                arquivo = os.path.join(caminho,"frame-{:06d}.jpg".format(conta))
                print arquivo
                #cv2.imwrite(arquivo,frame)
            cv2.imshow("Anotado",reduzido)        
