...
parser.add_argument("-u", "--mdburl",
default="mongodb://192.168.10.236:37027",
    help="URL de conecao ao banco de dados")

parser.add_argument('--output', type=str,
                    help="diretorio de gravacao da sequencia de frames",
                    default='/iscsi/videos/sequence')
...
if __name__=="__main__":
    print args.video
    # inicia a leitura do vido armazenado
    cap = cv2.VideoCapture(args.video)
    parts =  args.video.split('-')
    npart = parts[1].split('.')
    # caminho fisico no sistema de arquivo da maquina 192.168.10.236
    caminho = os.path.join(args.output,"p.{}".format(npart[0]))
    # inicializacao para colecao 
    princ = "p_{}_frame".format(npart[0])
        ...
        if ret:
            ...
            for x0,y0,x1,y1 in rectpts:
                ...
                if (x1-x0)/fatorg > 100 or (y1-y0)/fatorg > 100:
                    grava = True
            if grava:
                arquivo = "frame-{:07d}.jpg".format(conta)
                camiarquivo = os.path.join(caminho,arquivo)
                #print camiarquivo
                cv2.imwrite(camiarquivo,frame)
                info={}
                info['faleIn']=arquivo
                info['rects']={}
                for idxr,rect in enumerate(rectpts):
                    #transforma a regia reduzida no tamanho orignal 
                    rect=simplesVetor(np.array(rect)/fatorg)
                    info['rects']["r{}".format(idxr)]=rect
                colecao.insert_one(info)
            if conta % 2000 == 0 and conta >0:
                dt = clock() - t
                fps = 2000.0/dt
                #cv2.imshow("Resultado",frame)
                print "Time {} s e {} fps at frame {}".format(dt,fps,conta)
                t = clock()
            conta += 1
        else:
            contaF += 1
