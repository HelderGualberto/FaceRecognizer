...
def drawResult(listaRel,listaF):
    ...
    for idxr,p in listaF.listHistory:        
        ...
        if img is None:
            print "{} in {} not found".format(listaRel[idxr][u'fileOut'],listaRel[idxr][u'rectPos'])
            continue
...
def updateSeq(listaRel,listaF):
    for idxr,p in listaF.listHistory:
        colecao.update({'_id':listaRel[idxr][u'_id']}, {"$set": {'trackSeq':listaF.identity}}, upsert=False)
...
if __name__=="__main__":
    ...
    print princ
    ret = colecao.find().sort([(u'fileIn',pymongo.ASCENDING)])
    ...
    for item in ret:
        ...
        if idx == previdx:
            rects.append((conta,item['rectPos']))
            listaRel[conta]=item
        else:
