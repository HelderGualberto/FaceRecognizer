...
#
# Script para analisar em stream de video tecnicas de detecao e ratreamento
#
#
...
def separaOddEven(img):
    [h,w,c] = img.shape
    dx = w/5
    par = np.zeros((h/2,dx,3),np.uint8)
    impar = np.zeros((h/2,dx,3),np.uint8)
    x0 = 2*w/5
    for iy in range(0,h/2-h/6):
        par[iy,:]=img[2*iy,x0:x0+dx]
        impar[iy,:]=img[2*iy+1,x0:x0+dx]
...
class Sujeito:
    def __init__(self,identity):
        ...
        # lista de objetos da classe Face
        self.listaFaces = []
    ...
    def adicionaSeProx(self,face,tdis=0.09,tang=0.15):
        for internof in self.listaFaces:
            ...
            if diffangh < tang and diffangv <tang and distancia <tdis:
                        ...
                        if disff > 25.0:
                            ...
                            if idxB > maxBlurIdx:
                                ...
                                if ret:
                                    ...
                                    if ret:
                                        if not processaSujeitos(identity,listaSujeitos,face):
                                            identity += 1
                                        face.arquivo = "camara_v{:02d}_i{:03d}_f{:05d}.jpg".format(contaArq,face.identity,contaFrame)
