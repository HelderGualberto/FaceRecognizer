        ...
        for pimagem in paths.list_images(mypath):
            ...
            for x1, y1, x2, y2 in rects:
                ...
                for bb in bbs:
                    ...
                    cv2.imshow("Pontos",rgbFrame)
                    key = cv2.waitKey(0)
                    classifica =0 if key == 1048624 else 1
                    caminhos = pimagem.split("/")
                    csvinfo = "{};{};{:5.3f};{:5.3f};{:5.1f};{};{};\n".format(
                            partes2[0],partes2[1],angcab,angvcab,
                            idxB,caminhos[len(caminhos)-1],classifica)
                    csvdb.write(csvinfo)
                    print csvinfo
                    #rep = net.forward(alignedFace)
                    RepUtil.gravaJSON(partes2[0],partes2[1],angcab,angvcab,idxB,caminhos[len(caminhos)-1],classifica,jsondb)
                    #info = { 'timestamp': partes2[0],
                    #        'name':partes2[1],
                    #        'anghori': angcab,
                    #        'angvert':angvcab,
                    #        'bluridx':idxB,
                    #        'filename':caminhos[len(caminhos)-1],
                    #        'isok':classifica}
                    #jsondb.write(json.dumps(info,separators=(',',':'))+"\n\r")
                    if os.path.exists(pimagem):
                        os.rename(pimagem,mypathproc+'/'+caminhos[len(caminhos)-1])
                    else:
                        print 'Arquivo copiado ?',pimagem
