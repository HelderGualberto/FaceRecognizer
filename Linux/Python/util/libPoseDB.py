...
# Data: 2016/10/05
#       2017/03q29 - remover entradas anteriores do logit 
...
class MongoConn:
    ...
    def salvaLogit(self,logreg,nomearq='logreg.pkl',anglim=30,isp5=False):
        ...
        ret = self.db.fs.files.find({'filename':arquivo})
        for fid in ret:            
            if self.fs.exists(fid['_id']):
                self.fs.delete(fid['_id'])
                print("Removido id:{} filename:{}".format(fid['_id'],fid['filename']))
        #if fs.
